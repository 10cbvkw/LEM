...
###########################################################
#from collections import OrderedDict#添加一个库
###########################################################
...
#        1 z_t+1 = z_t +\epsilon
#
#        self.mu_epsilon = torch.nn.Parameter(torch.FloatTensor(1),requires_grad=True)
#        self.var_epsilon = torch.nn.Parameter(torch.FloatTensor(1),requires_grad=True)
#
#        2 z_t+1 = Az_t + \epsilon
#
#        dic_layers = OrderedDict()
#        dic_layers['linear'] = nn.Linear(self.z_dim, self.z_dim)
#        self.matrix_mean = nn.Sequential(dic_layers)
#        dic_layers = OrderedDict()
#        dic_layers['linear'] = nn.Linear(self.z_dim, self.z_dim)
#        self.matrix_var = nn.Sequential(dic_layers)
#
#        3 MLP 控制z_t->z_t+1 + \epsilon
#
#        dic_layers = OrderedDict()
#        dic_layers['linear1'] = nn.Linear(self.z_dim, 16)
#        dic_layers['linear2'] = nn.Linear(16, 32)
#        dic_layers['linear3'] = nn.Linear(32, self.z_dim)
#        self.mlp_mean = nn.Sequential(dic_layers)
#        dic_layers = OrderedDict()
#        dic_layers['linear1'] = nn.Linear(self.z_dim, 16)
#        dic_layers['linear2'] = nn.Linear(16, 32)
#        dic_layers['linear3'] = nn.Linear(32, self.z_dim)
#        self.mlp_var = nn.Sequential(dic_layers)
#   
#        4 GRU 控制z_t->z_t+1 + \epsilon
#
#         # 1. Gating Unit
#        dic_layers = OrderedDict()
...
#    1 z_t+1 = z_t +\epsilon
#
#    def generate_z1(self, z_tm1):
#        return self.mu_epsilon, self.var_epsilon
# 
#    2 z_t+1 = Az_t + \epsilon
#
#    def generate_z2(self, z_tm1):
#        z_mean_p = self.matrix_mean(z_tm1)
#        z_var_p = self.matrix_var(z_tm1)
#        z_logvar_p = torch.log(z_var_p)
#        return z_mean_p + self.mu_epsilon, z_logvar_p + self.var_epsilon
#
#    3 MLP控制z_t->z_t+1
#
#    def generate_z3(self, z_tm1):
#        z_mean_p = self.mlp_mean(z_tm1)
#        z_var_p = self.mlp_var(z_tm1)
#        z_logvar_p = torch.log(z_var_p)
#        return z_mean_p, z_logvar_p
#
#    4 GRU 控制z_t->z_t+1
#        
#    def generate_z4(self, z_tm1):
