...
################################################################################################################
#
# def train_batch(model, lattice, y, yt, rot, trans, optim, beta, device, beta_control=None, tilt=None, ctf_params=None, yr=None, use_amp=False, scaler=None):
#
################################################################################################################
    optim.zero_grad()
    model.train()
...
################################################################################################################
#
#            z_0 = torch.zeros(1, z.shape[1]).to(device)
#            z_tm1 = torch.cat([z_0, z[:-1,:]], 0)
#            z_mu_p, z_logvar_p = model.generate_z(z_tm1)
#            loss, gen_loss, kld = loss_function_1(z_mu, z_logvar, z_mu_p, z_logvar_p, y, yt, y_recon, mask, beta, y_recon_tilt, beta_control)
#
################################################################################################################
...
########################################################################################################
#
#def loss_function_1(z_mu, z_logvar, z_mu_p, z_logvar_p, y, yt, y_recon, mask, beta, y_recon_tilt=None, beta_control=None):
#    # reconstruction error
#    use_tilt = yt is not None
#    B = y.size(0)
#    gen_loss = F.mse_loss(y_recon, y.view(B,-1)[:, mask])
...
#        gen_loss = .5*gen_loss + .5*F.mse_loss(y_recon_tilt, yt.view(B,-1)[:,mask])
#    # latent loss
#    kld = torch.mean(-0.5 * torch.sum(z_logvar - z_logvar_p - torch.div(z_logvar.exp() + (z_mu - z_mu_p).pow(2), z_logvar_p.exp()+1e-10), dim=1), dim=0)
#    # total loss
#    if beta_control is None:
#        loss = gen_loss + beta*kld/mask.sum().float()
#    else:
...
#################################################################################################################
#
#            loss, gen_loss, kld = train_batch(model, lattice, y, yt, rot, tran, optim, beta, args.beta_control, tilt, ctf_params=ctf_param, yr=yr, use_amp=args.amp, scaler=scaler, device = device)
#
#################################################################################################################
